...
// --- Main Component ---
const ProgressBar = ({ status, grantId, onStatusUpdate }) => {
  ...
  const isRejected =
    currentStatusInfo.step === 4 && status === GrantStatus.REJECTED;

  // Handle step click
  const handleStepClick = (stepName) => {
    if (onStatusUpdate && stepToStatus[stepName]) {
      onStatusUpdate(grantId, stepToStatus[stepName]);
    }
  };
        ...
        {steps.map((step, index) => (
          <span
            ...
            className={`${
              ...
              onStatusUpdate
                ? "cursor-pointer hover:text-primary dark:hover:text-dark-primary transition-colors"
                : ""
            }`}
          >
            {step}
