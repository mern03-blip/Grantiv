...
import { getOrganizationSubscription, getSubscriptionStatus } from "../../../api/endpoints/payment";
...
const Notification = () => {
  ...
  const { data: subscriptionData, isLoading: isLoadingSubscription } = useQuery({
   ...
   const { data: organizationData } = useQuery({
    ...
    const organization = organizationData?.data?.notificationSettings || {};

  // Keep local `settings` in sync with fetched organization settings
  useEffect(() => {
    if (organization && Object.keys(organization).length) {
      setSettings((prev) => ({ ...prev, ...organization }));
    }
  }, [organization]);
  ...
  const mutation = useMutation({
    ...
    onSuccess: (_data, variables) => {
      const { settingKey, isEnabled } = variables || {};
      queryClient.setQueryData(["organization-details"], (old) => {
        const oldData = old || {};
