...
const MyGrants = () => {
  ...
  return (
    <div>
      <h2 className="text-3xl font-bold text-night dark:text-dark-text mb-2 font-heading">My Grant Applications</h2>
      <p className="text-night/60 dark:text-dark-textMuted mb-8">Track the status and progress of all your grant applications.</p>
      <div className="space-y-6">
        {ALL_GRANTS.length > 0 ? ALL_GRANTS.map(grant => (
          <div key={grant.id} className="bg-white dark:bg-dark-surface p-6 rounded-lg border border-mercury dark:border-dark-border">
            <div className="flex justify-between items-center mb-4">
              <div>
                <h4 className="text-xl font-bold text-night dark:text-dark-text font-heading">{grant.title}</h4>
                <p className="text-sm text-night/60 dark:text-dark-textMuted">{grant.funder}</p>
              </div>
              <motion.button
          ...
          <div className="text-center p-8 bg-white dark:bg-dark-surface rounded-lg border border-mercury dark:border-dark-border">
            <p className="text-night/60 dark:text-dark-textMuted">You haven't started any applications yet.</p>
          </div>
        )}
      </div>
    </div>
